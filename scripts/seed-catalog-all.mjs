// Adds the same batch of catalog items (from seed-catalog-demo.mjs) to every
// user account in the local db, per user request ("sab pr lagado").
// Run with: node --env-file=.env scripts/seed-catalog-all.mjs
...
async function main() {
  ...
  for (const user of users) {
    await prisma.product.createMany({
      data: items.map((item) => ({ userId: user.id, ...item })),
    });
    console.log(`+ ${items.length} items -> ${user.email}`);
  }
  console.log(`\nDone. ${users.length} accounts updated.`);
}
